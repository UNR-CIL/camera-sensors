...
{
    CGPoint pointInView = [gestureRecognizer locationInView:_zoomView];
    
    CGFloat newZoomScale = self.zoomScale * 1.5f;
    if (newZoomScale > self.maximumZoomScale) {
        newZoomScale = 0;
    }
    else {
...
- (void)displayImage:(UIImage *)image
{
    ...
    [_zoomView setContentScaleFactor:0];
    [self addSubview:_zoomView];
    
    
    [_zoomView setUserInteractionEnabled:YES];
...
- (void)setMaxMinZoomScalesForCurrentBounds
{
    ...
    CGFloat maxScale = 1.0; // Force it to be 1.0 so it can zoom
